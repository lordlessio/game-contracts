...
// File: contracts/LdbERC721Token.sol
 ...
 * @dev see https://github.com/ethereum/EIPs/blob/master/EIPS/eip-721.md
 */
contract LdbERC721Token is ERC721, ERC721BasicToken {
  // Token name
  ...
  /**
   * @dev Returns a LDB-NFT detail by tokenId
   * @dev Throws if the token ID does not exist. May return an empty string.
   * @param _tokenId uint256 ID of the token to query
   */
  ...
  /**
   * @dev Internal function to set the LDB instance for a given token
   * @dev Reverts if the params format error
   * @param _tokenId uint256 ID of the token to set its LDB
   * @param _lat uint256 to assign
   */
  function _setLdbNFT(uint256 _tokenId, uint32 _lat, uint32 _lon) internal {
    
    require(_tokenId == uint256(_tokenId));
    require(_lat == uint256(_lat));
    require(_lon == uint256(_lon));
  ...
  function _burn(address _owner, uint256 _tokenId) internal {
    ...
    // // Clear metadata (if any)
    // if (bytes(tokenLDBs[_tokenId]).tokenId != 0) {
    //   delete tokenLDBs[_tokenId];
    // } 
